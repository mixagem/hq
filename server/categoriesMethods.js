...
export function updateCategory(req, res) {
  ...
  let db = new sqlite3.Database('./mhq.db', sqlite3.OPEN_READWRITE, (err) => {
    if (err) { console.error(err.message); }
    console.log('[C6] updating category "' + CATEGORY.title + '"');
  });

  db.serialize(() => {
    db.run(`UPDATE categories SET title='${CATEGORY.title}', type='${CATEGORY.type}', icon='${CATEGORY.icon}', bgcolor='${CATEGORY.bgcolor}', textcolor='${CATEGORY.textcolor}', active='${CATEGORY.active}', catorder='${CATEGORY.catorder}' WHERE id='${CATEGORY.id}'`, (err, resp) => { err ? console.error(err.message) : console.log('Category sucessfully updated.'); });
    db.run(`DELETE FROM subcategories WHERE maincatid='${CATEGORY.id}'`, (err, resp) => { err ? console.error(err.message) : console.log('[C6b] updating "' + CATEGORY.title + '" sub-categories'); });
