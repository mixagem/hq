...
export function dailySubCatDetails(req, res) {
  ...
  console.log(new Date(dataIniMilli).toLocaleString('pt'))
  console.log(new Date(dataFiMilli).toLocaleString('pt'))
  let dailyMovments = [];
...
export function monthlySnapshots(req, res) {
  ...
  function generateSnapshots() {
    let query = `SELECT * FROM treasurylog WHERE date <= ${dataFiMilli} AND date >= ${dataIniMilli}`
    let queryExtra = '';


    categoryList.forEach((category, i) => {
      i === 0 ? queryExtra += ` AND cat='${category}'` : queryExtra += ` OR cat='${category}'`;
      generatedCategorySnapshots[`${category}`] = new Array(req.body.monthdays).fill(0)
    });

    subCategoryList.forEach((subcategory, i) => {
      i === 0 ? queryExtra += ` AND subcat='${subcategory}'` : queryExtra += ` OR subcat='${subcategory}'`;
      generatedSubCategorySnapshots[`${subcategory}`] = new Array(req.body.monthdays).fill(0)
    });
    ...
    monthlyMovments.forEach(movement => {
      if (movement.type === 'expense') {
        generatedCategorySnapshots[`${movement.cat}`][new Date(movement.date).getDate() - 1] -= movement.value
        generatedSubCategorySnapshots[`${movement.subcat}`][new Date(movement.date).getDate() - 1] -= movement.value
      } else {
        generatedCategorySnapshots[`${movement.cat}`][new Date(movement.date).getDate() - 1] += movement.value
        generatedSubCategorySnapshots[`${movement.subcat}`][new Date(movement.date).getDate() - 1] += movement.value
      }
