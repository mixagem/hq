...
export class DeleteCategoryConfirmationModalComponent {
  ...
  deleteCategory(): void {
    ...
    // }
    for (const TLOG in this._treasuryService.tlogEnum) { if (this._treasuryService.tlogEnum[TLOG].cat === this.categoriesService.activePreviewCategory.id) { return this._mhqSnackbarService.triggerMHQSnackbar(false, 'report', this.categoriesService.activePreviewCategory.title, ['Não é possível remover a categoria ', ', devido à existência de movimentos associados a esta.']); } }
